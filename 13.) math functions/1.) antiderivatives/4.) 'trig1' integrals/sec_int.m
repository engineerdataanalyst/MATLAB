function [F f tF dF sF cF] = sec_int(n, a, b, x, options)
  ...
  if isequal(Method, "all")
    for k = ["one" "two"]
      Args = [args {'Method' k}];
      [F.(k) f.(k) tF.(k) dF.(k) sF.(k) cF.(k)] = sec_int(Args{:});
    end
    return;
  end
...
function [Fp fp dFp] = persistent_fun
  ...
  S = heaviside(k-1);
  sublist = [1/(a^S*B); (-1)^N];
  subvals = [-1/((-a)^S*B); 1/sublist(2)];
  %% integral calculations
  [Fp fp] = cos_int(-n, a, b, x);
  for p = ["one" "two"]
    Fp.(p)(n, a, b, x) = order_branches(Fp.(p), [1 7:-1:5 3:4 2]);
    fp.(p)(n, a, b, x) = fp.(p);
    if p == "one"
      Fp.(p) = subs(Fp.(p), sublist(1), subvals(1));
    else
      Fp.(p) = subs(Fp.(p), sublist, subvals);
    end
