function [F f tF dF sF cF] = cosx_int(n, a, b, x)
  % - formula for this integral:
  % --------------------------
  %  /
  %  | (a*x+b)^n*cos(a*x+b) dx
  %  /
  % --------------------------
...
function Fp = n_neg_int(n, a, b, x)
  ...
  N.cos = piecewise(isodd(n), -n/2-1/2, ...
                    iseven(n), -n/2);
  P = -expression(N.cos, [2 1]);
  Arg = [k-1; P];
  assume(Arg(2:3), 'real');
  S = [heaviside(Arg(1)); integer(Arg(2:3))];
  clearassum;
  %% term calculations
  K.sin = (-1)^(k+1)/(a^S(1)*npermk(-n-1, 2*k));
  K.cos = (-1)^k/(a^S(1)*npermk(-n-1, 2*k-1));
  K.sinint = (-1)^P(1)/(a*factorial(-n-1));
  K.cosint = (-1)^P(2)/(a*factorial(-n-1));
  Term.sin = K.sin*axb^(n+2*k)*sin(axb);
  Term.cos = K.cos*axb^(n+2*k-1)*cos(axb);
  Term.sinint = K.sinint*sinint(axb)*S(2);
  Term.cosint = K.cosint*cosint(axb)*S(3);
...
function Fp = n_nonpos_half(n, a, b, x)
  ...
  N.sin = piecewise(isint(-n/2-3/4), -n/2-3/4, ...
                    isint(-n/2-1/4), -n/2-1/4);
  N.cos = piecewise(isint(-n/2-1/4), -n/2-1/4, ...
                    isint(-n/2+1/4), -n/2+1/4);
  P = -expression(N.sin);
  Arg = [k-1; P; n+[1; 2*k+[1; 0]]];
  assume(Arg(2:3), 'real');
  S = [heaviside(Arg(1)); integer(Arg(2:3))];
  G = gamma(Arg(4:6));
  clearassum;
...
function Fp = n_nonneg_int(n, a, b, x)
  ...
  S = heaviside(k-1);
  %% term calculations
  K.sin = (-1)^(k+1)*npermk(n, 2*k-2)/a^S;
  K.cos = (-1)^(k+1)*npermk(n, 2*k-1)/a^S;
  Term.sin = K.sin*axb^(n-2*k+2)*sin(axb);
  Term.cos = K.cos*axb^(n-2*k+1)*cos(axb);
  %% integral calculation
...
function Fp = n_nonneg_half(n, a, b, x)
  ...
  N.cos = piecewise(isint(n/2-1/4), n/2-1/4, ...
                    isint(n/2+1/4), n/2+1/4);
  P = 3*n/2+[-3/4; 3/4];
  Arg = [k-1; expression(N.cos); -n+[2*k+[-2; -1]; 0]];
  assume(Arg(2:3), 'real');
  S = [heaviside(Arg(1)); integer(Arg(2:3))];
  G = gamma(Arg(4:6));
  clearassum;
  %% term calculations
  K.sin = (-1)^(k+1)*G(1)^S(1)/(a*G(3))^S(1);
