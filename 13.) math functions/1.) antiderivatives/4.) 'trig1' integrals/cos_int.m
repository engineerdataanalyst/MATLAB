function [F f tF dF sF cF] = cos_int(n, a, b, x, options)
  ...
  if isequal(Method, "all")
    for k = ["one" "two"]
      Args = [args {'Method' k}];
      [F.(k) f.(k) tF.(k) dF.(k) sF.(k) cF.(k)] = cos_int(Args{:});
    end
    return;
  end
...
function [Fp fp dFp] = persistent_fun
  ...
  Fp.two(7) = n_odd_pos_two(n, a, b, x);
  % ---------------------------------
...
function Fp = n_neg_even_two(n, a, b, x)
  ...
  G = gamma(Arg).^[-1; S];
  %% term calculations
  K.sincos = (-1)^(k+1)*prod(G(1:2))*npermk(N, k)/(a^S*(n+2*k));
  K.tan = (-1)^(-N-1)*G(2)*factorial(N)/(2*a*sqrt(sympi));
  K.tan = subs(K.tan, k, 1);
  Term.sincos = K.sincos*sin(axb)^S*cos(axb)^(n+2*k-1);
  Term.tan = K.tan*sin(axb)/cos(axb);
  %% integral calculation
  Fp = symsum(Term.sincos, k, 1, N)+Term.tan;
end
% =
function Fp = n_nonpos_half(n, a, b, x)
  ...
  K.E = subs(K.E, k, 1);
  K.F = subs(K.F, k, 1);
  Term.sincos = K.sincos*sin(axb)^S(1)*cos(axb)^(n+2*k-1);
...
function Fp = n_neg_odd(n, a, b, x)
  ...
  G = gamma(Arg).^[1; -S];
  %% term calculations
  K.sincos = (-1)^(k+1)*prod(G(1:2))/(a^S*(n+2*k)*npermk(N, k));
  K.log_sec_tan = (-1)^(-N)*sqrt(sympi)*G(2)/(a*factorial(N));
  K.log_sec_tan = subs(K.log_sec_tan, k, 1);
  Term.sincos = K.sincos*sin(axb)^S*cos(axb)^(n+2*k-1);
  Term.log_sec_tan = K.log_sec_tan*log(sec(axb)+tan(axb));
  %% integral calculation
  Fp = symsum(Term.sincos, k, 1, N)+Term.log_sec_tan;
end
% =
function Fp = n_nonneg_even_one(n, a, b, x)
  ...
  S = heaviside(k-1);
  %% term calculations
  K.sin = nchoosek(n, k-1)/(2^((n-1)*S)*a^S*(n-2*k+2));
  K.x = nchoosek(n, N)/2^n;
  Term.sin = K.sin*sin(axb*(n-2*k+2));
  Term.x = K.x*x;
  %% integral calculation
...
function Fp = n_nonneg_even_two(n, a, b, x)
  ...
  G = gamma(Arg).^[1; -S];
  %% term calculations
  K.sincos = (-1)^(k)*prod(G(1:2))/(a^S*npermk(N, k)*(n-2*k+1));
...
function Fp = n_pos_odd_one(n, a, b, x)
  ...
  S = heaviside(k-1);
  %% term calculations
  K = (-1)^(k+1)*nchoosek(N-1, k-1);
  Term = K*sin(axb)^(2*k-1)/(a^S*(2*k-1));
  %% integral calculation
  Fp = symsum(Term, k, 1, N);
end
% =
function Fp = n_odd_pos_two(n, a, b, x)
  % ---------------------------------
  % - helper function for calculating
  %   the persistent integral
  %   for the following case:
  ...
  G = gamma(Arg).^[-1; S];
  %% term calculations
  K.sincos = (-1)^k*prod(G(1:2))*npermk(N, k)/(a^S*(n-2*k+1));
  K.sin = (-1)^(N+1)*G(2)*factorial(N)/(2*a*sqrt(sympi));
  K.sin = subs(K.sin, k, 1);
  Term.sincos = K.sincos*sin(axb)^S*cos(axb)^(n-2*k+1);
  Term.sin = K.sin*sin(axb);
  %% integral calculation
  Fp = symsum(Term.sincos, k, 1, N)+Term.sin;
end
% =
