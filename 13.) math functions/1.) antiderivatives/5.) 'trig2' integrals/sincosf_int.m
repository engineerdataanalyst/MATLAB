function [F f tF dF sF cF] = sincosf_int(a, b, alpha, beta, delta, epsilon, x)
  % - formula for this integral:
  % -----------------------------------------
  %  /
  %  |  alpha*sin(a*x+b)+beta*cos(a*x+b)
  %  | ----------------------------------- dx
  %  | delta*sin(a*x+b)+epsilon*cos(a*x+b)
  %  /
  % -----------------------------------------
...
function [Fp fp dFp] = persistent_fun
  ...
  [a b alpha beta delta epsilon x] = deal(sym('a'), sym('b'), ...
                                          ...
                                          sym('x'));
  num = alpha*sin(a*x+b)+beta*cos(a*x+b);
  den = delta*sin(a*x+b)+epsilon*cos(a*x+b);
...
function Fp = a_not_zero(a, b, alpha, beta, delta, epsilon, x)
  ...
  axb = a*x+b;
  %% constant calculations (case 1: [delta == 1i*epsilon])
  K.one = sym.zeros(2,1);
  K.one(1) = -(alpha-beta*1i)/(4*a*delta);
  K.one(2) = (alpha+beta*1i)/(2*delta);
  %% term calculations (case 1: [delta == 1i*epsilon])
  Term.one = sym.zeros(2,1);
  Term.one(1) = 1i*exp(-2i*axb);
  Term.one(2) = x;
  %% integral calculation (case 1: [delta == 1i*epsilon])
  Fp.one = sum(K.one.*Term.one);
  %% constant calculations (case 2: [delta == -1i*epsilon])
  K.two = sym.zeros(2,1);
  K.two(1) = (alpha+beta*1i)/(4*a*delta);
  K.two(2) = (alpha-beta*1i)/(2*delta);
  %% term calculations (case 2: [delta == -1i*epsilon])
  Term.two = sym.zeros(2,1);
  Term.two(1) = 1i*exp(2i*axb);
  Term.two(2) = x;
  %% integral calculation (case 2: [delta == -1i*epsilon])
  ...
  Term.three(1) = log(delta*sin(axb)+epsilon*cos(axb))/a;
  Term.three(2) = x;
  ...
  Fp = piecewise(delta == 1i*epsilon,    Fp.one, ...
                 delta == -1i*epsilon,   Fp.two, ...
