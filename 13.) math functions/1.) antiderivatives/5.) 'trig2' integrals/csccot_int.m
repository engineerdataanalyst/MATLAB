function [F f tF dF sF cF] = csccot_int(n, p, a, b, x)
  % - formula for this integral:
  % -------------------------------
  %  /
  %  | csc(a*x+b)^n*cot(a*x+b)^n dx
  %  /
  % -------------------------------
...
function [Fp fp dFp] = persistent_fun
  ...
  [Fp fp.non_handle] = sincos_int(-(n+p), p, a, b, x);
  Fp(n, p, a, b, x) = Fp;
...
function h = handle_fun(Fp, fp, dFp_args, h_type)
  ...
  switch h_type
    case "fp"
      h = rewrite(fp.non_handle, 'sincos');
      h = simplify(h, IAC{:});
      h = h(dFp_args{:});
    case "dFp"
      func = @(arg, var) diff(rewrite(arg, 'sincos'), var);
