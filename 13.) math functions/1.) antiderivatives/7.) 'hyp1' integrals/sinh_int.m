function [F f tF dF sF cF] = sinh_int(n, a, b, x, options)
  ...
  if isequal(Method, "all")
    for k = ["one" "two"]
      Args = [args {'Method' k}];
      [F.(k) f.(k) tF.(k) dF.(k) sF.(k) cF.(k)] = sinh_int(Args{:});
    end
    return;
  end
...
function [Fp fp dFp] = persistent_fun
  ...
  n_is_pos_half = isint(n-1/2, 'Type', 'positive or zero');
...
function Fp = n_neg_even_one(n, a, b, x)
  ...
  S = heaviside(k-1);
  %% term calculations
  K = (-1)^(k+1)*nchoosek(N-1, k-1)/(a^S*(n+2*k-1));
  Term = K*coth(axb)^(-n-2*k+1);
...
function Fp = n_neg_even_two(n, a, b, x)
  ...
  G = gamma(Arg).^[-1; S];
  %% term calculations
  K.sinhcosh = prod(G(1:2))*npermk(N, k)/((-a)^S*(n+2*k));
  K.coth = G(2)*factorial(N)/(2*a*sqrt(sympi));
  K.coth = subs(K.coth, k, 1);
  Term.sinhcosh = K.sinhcosh*sinh(axb)^(n+2*k-1)*cosh(axb)^S;
  Term.coth = K.coth*cosh(axb)/sinh(axb);
  %% integral calculation
  Fp = symsum(Term.sinhcosh, k, 1, N)+Term.coth;
end
% =
function Fp = n_nonpos_half(n, a, b, x)
  ...
  elliptic_args{1} = sympi/4-axb*1i/2;
  elliptic_args{2} = 2;
  N = -n/2+[-1/2; -1; -1/2; 1/4; -1/4; 5/2; 3/2];
  ...
  clearassum;
  %% term calculations
  K.sinhcosh = (-1)^(k+1)*prod(G(1:4))/(a^S(1)*(n+2*k));
  K.E = (-1)^N(6)*sqrt(sym(2))*sympi*prod(G(3:4))*S(2)/(2*a*G(5));
  K.F = -(-1)^N(7)*sqrt(sym(2))*prod(G(3:5))*S(3)/(a*sympi);
  K.E = subs(K.E, k, 1);
  K.F = subs(K.F, k, 1);
  Term.sinhcosh = K.sinhcosh*sinh(axb)^(n+2*k-1)*cosh(axb)^S(1);
  Term.E = K.E*ellipticE(elliptic_args{:});
  Term.F = K.F*ellipticF(elliptic_args{:});
  %% integral calculation
  Fp = sym.zeros(2,1);
  Fp(1) = symsum(Term.sinhcosh, k, 1, N(8));
  Fp(2) = Term.E+Term.F;
  Fp = sum(Fp);
end
% =
function Fp = n_neg_odd(n, a, b, x)
  ...
  G = gamma(Arg).^[1; -S];
  %% term calculations
  K.sinhcosh = prod(G(1:2))/((-a)^S*(n+2*k)*npermk(N, k));
  K.log_csch_coth = -sqrt(sympi)*G(2)/(a*factorial(N));
  K.log_csch_coth = subs(K.log_csch_coth, k, 1);
  Term.sinhcosh = K.sinhcosh*sinh(axb)^(n+2*k-1)*cosh(axb)^S;
  Term.log_csch_coth = K.log_csch_coth*log(csch(axb)+coth(axb));
  %% integral calculation
  Fp = symsum(Term.sinhcosh, k, 1, N)+Term.log_csch_coth;
end
% =
function Fp = n_nonneg_even_one(n, a, b, x)
  ...
  N = n/2;
  %% term calculations
  K.sinh = (-1)^(n+k+1)*nchoosek(n, k-1)/(2^((n-1)*S)*a^S*(n-2*k+2));
  K.x = (-1)^N*nchoosek(n, N)/2^n;
  Term.sinh = K.sinh*sinh((n-2*k+2)*axb);
  Term.x = K.x*x;
  %% integral calculation
  Fp = symsum(Term.sinh, k, 1, N)+Term.x;
end
% =
function Fp = n_nonneg_even_two(n, a, b, x)
  ...
  G = gamma(Arg).^[1; -S];
  %% term calculations
  K.sinhcosh = prod(G(1:2))/((-a)^S*npermk(N, k)*(n-2*k+1));
  K.x = sqrt(sympi)*G(2)/factorial(N);
  K.x = subs(K.x, k, 1);
  Term.sinhcosh = K.sinhcosh*sinh(axb)^(n-2*k+1)*cosh(axb)^S;
  Term.x = K.x*x;
  %% integral calculation
  Fp = symsum(Term.sinhcosh, k, 1, N)+Term.x;
end
% =
function Fp = n_nonneg_half(n, a, b, x)
  ...
  clearassum;
  %% term calculations
  K.sinhcosh = (-1)^(k+1)*prod(G(1:4))/(a^S(1)*(n-2*k+1));
  K.E = (-1)^(N(6))*sqrt(sym(2))*sympi*prod(G(3:4))*S(2)/(2*a*G(5));
  K.F = -(-1)^(N(7))*sqrt(sym(2))*prod(G(3:5))*S(3)/(a*sympi);
  K.E = subs(K.E, k, 1);
  K.F = subs(K.F, k, 1);
...
function Fp = n_pos_odd_one(n, a, b, x)
  ...
  N = (n-1)/2;
  %% term calculations
  K = (-1)^(k+1)*nchoosek(N, k-1)/(a^S*(n-2*k+2));
  Term = K*cosh(axb)^(n-2*k+2);
  %% integral calculation
  Fp = symsum(Term, k, 1, N+1);
end
% =
function Fp = n_pos_odd_two(n, a, b, x)
  ...
  G = gamma(Arg).^[-1; S];
  %% term calculations
  K.sinhcosh = prod(G(1:2))*npermk(N, k)/((-a)^S*(n-2*k+1));
  K.cosh = -G(2)*factorial(N)/(2*a*sqrt(sympi));
  K.cosh = subs(K.cosh, k, 1);
  Term.sinhcosh = K.sinhcosh*sinh(axb)^(n-2*k+1)*cosh(axb)^S;
  Term.cosh = K.cosh*cosh(axb);
  %% integral calculation
  Fp = symsum(Term.sinhcosh, k, 1, N)+Term.cosh;
end
% =
