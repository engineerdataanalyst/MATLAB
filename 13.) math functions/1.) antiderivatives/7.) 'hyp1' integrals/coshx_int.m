function [F f tF dF sF cF] = coshx_int(n, a, b, x)
  % - formula for this integral:
  % ---------------------------
  %  /
  %  | (a*x+b)^n*cosh(a*x+b) dx
  %  /
  % ---------------------------
...
function Fp = n_neg_int(n, a, b, x)
  ...
  k = sym('k');
  axb = a*x+b;
  N.sinh = piecewise(isodd(n), -n/2-1/2, ...
                     iseven(n), -n/2-1);
  N.cosh = piecewise(isodd(n), -n/2-1/2, ...
                     iseven(n), -n/2);
  P = -2*expression(N.cosh, [2 1]);
  Arg = [k-1; P];
  assume(Arg(2:3), 'real');
  S = [heaviside(Arg(1)); integer(Arg(2:3)/2)];
  clearassum;
  %% term calculations
  K.sinh = -1/(a^S(1)*npermk(-n-1, 2*k));
  K.cosh = -1/(a^S(1)*npermk(-n-1, 2*k-1));
  K.sinhint = (-1)^P(1)/(a*factorial(-n-1));
  K.coshint = (-1)^P(2)/(a*factorial(-n-1));
  Term.sinh = K.sinh*axb^(n+2*k)*sinh(axb);
  Term.cosh = K.cosh*axb^(n+2*k-1)*cosh(axb);
  Term.sinhint = K.sinhint*sinhint(axb)*S(2);
  Term.coshint = K.coshint*coshint(axb)*S(3);
  %% integral calculation
  Fp = sym.zeros(2,1);
  Fp(1) = symsum(Term.sinh, k, 1, N.sinh)+symsum(Term.cosh, k, 1, N.cosh);
  Fp(2) = Term.sinhint+Term.coshint;
  Fp = sum(Fp);
end
% =
function Fp = n_nonpos_half(n, a, b, x)
  ...
  k = sym('k');
  axb = a*x+b;
  fresnel_arg = sym(-1)^(1/4)*sqrt(sym(2))*sqrt(axb)/sqrt(sympi);
  N.sinh = piecewise(isint(-n/2-3/4), -n/2-3/4, ...
                     isint(-n/2-1/4), -n/2-1/4);
  N.cosh = piecewise(isint(-n/2-1/4), -n/2-1/4, ...
                     isint(-n/2+1/4), -n/2+1/4);
  P = sym([1/4; 3/4+1]);
  Arg = [k-1; -expression(N.cosh, [2 1]); n+[1; 2*k+[1; 0]]];
  assume(Arg(2:3), 'real');
  S = [heaviside(Arg(1)); integer(Arg(2:3))];
  G = gamma(Arg(4:6));
  clearassum;
  %% term calculations
  K.sinh = -G(1)^S(1)/(a^S(1)*G(2));
  K.cosh = G(1)^S(1)/(a^S(1)*G(3));
...
function Fp = n_nonneg_int(n, a, b, x)
  ...
  S = heaviside(k-1);
  %% term calculations
  K.sinh = npermk(n, 2*k-2)/a^S;
  K.cosh = -npermk(n, 2*k-1)/a^S;
  Term.sinh = K.sinh*axb^(n-2*k+2)*sinh(axb);
  Term.cosh = K.cosh*axb^(n-2*k+1)*cosh(axb);
  %% integral calculation
  Fp = symsum(Term.sinh, k, 1, N)+symsum(Term.cosh, k, 1, N);
end
% =
function Fp = n_nonneg_half(n, a, b, x)
  ...
  N.cosh = piecewise(isint(n/2-1/4), n/2-1/4, ...
                     isint(n/2+1/4), n/2+1/4);
  P = n+[3/4 1/4];
  Arg = [k-1; expression(N.cosh); -n+[2*k+[-2; -1]; 0]];
  assume(Arg(2:3), 'real');
  S = [heaviside(Arg(1)); integer(Arg(2:3))];
  G = gamma(Arg(4:6));
  clearassum;
  %% term calculations
  K.sinh = G(1)^S(1)/(a*G(3))^S(1);
  K.cosh = G(2)^S(1)/(a*G(3))^S(1);
