function [F f tF dF sF cF] = sech_int(n, a, b, x, options)
  ...
  if isequal(Method, "all")
    for k = ["one" "two"]
      Args = [args {'Method' k}];
      [F.(k) f.(k) tF.(k) dF.(k) sF.(k) cF.(k)] = sech_int(Args{:});
    end
    return;
  end
...
function [Fp fp dFp] = persistent_fun
  ...
  B = -n-2*k+1;
  S = heaviside(k-1);
  sublist = [(-1)^k*cosh(axb)^B;
             (-1).^(-N+[0; -1/2]);
             1/(a^S*gamma(k+n/2))];
  subvals = [-(-1)^(k+1)*cosh(axb)^B;
             1./sublist(2:3);
             -1/((-a)^S*gamma(k+n/2))];
  %% integral calculations
  [Fp fp] = cosh_int(-n, a, b, x);
  for p = ["one" "two"]
