function [F f tF dF sF cF] = csch_int(n, a, b, x, options)
  ...
  if isequal(Method, "all")
    for k = ["one" "two"]
      Args = [args {'Method' k}];
      [F.(k) f.(k) tF.(k) dF.(k) sF.(k) cF.(k)] = csch_int(Args{:});
    end
    return;
  end
...
function [Fp fp dFp] = persistent_fun
  ...
  k = sym('k');
  S = heaviside(k-1);
  sublist = [(-1)^(k+1)*([n-2*k+1; n+2*k+[-1; -2]]).^-1;
             (-1).^(k-n+1);
             1./((-a).^S.*[gamma(k+n/2); gamma(n/2+1/2)^S]);
             (-1)^(k+1)*gamma(n/2)^S];
  subvals = [-sublist(1:3)*(-1)^k/(-1)^(k+1);
             (-1).^(k+n+1);
             -(-a).^S.*sublist(5:6)./a.^S;
             -sublist(7)*(-1)^k/(-1)^(k+1)];
  %% integral calculations
  [Fp fp] = sinh_int(-n, a, b, x);
  for p = ["one" "two"]
    ...
    if p == "one"
      ...
      Fp.(p) = subs(Fp.(p), sublist(5:7), subvals(5:7));
    end
