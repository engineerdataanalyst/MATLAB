function [F f tF dF sF cF] = cosh_int(n, a, b, x, options)
  ...
  if isequal(Method, "all")
    for k = ["one" "two"]
      Args = [args {'Method' k}];
      [F.(k) f.(k) tF.(k) dF.(k) sF.(k) cF.(k)] = cosh_int(Args{:});
    end
    return;
  end
...
function Fp = n_neg_even_one(n, a, b, x)
  ...
  S = heaviside(k-1);
  %% term calculations
  K = (-1)^(k+1)*nchoosek(N-1, k-1)/(a^S*(2*k-1));
  Term = K*tanh(axb)^(2*k-1);
...
function Fp = n_neg_even_two(n, a, b, x)
  ...
  G = gamma(Arg).^[-1; S];
  %% term calculations
  K.sinhcosh = (-1)^(k+1)*prod(G(1:2))*npermk(N, k)/(a^S*(n+2*k));
  K.tanh = (-1)^(-N-1)*G(2)*factorial(N)/(2*a*sqrt(sympi));
  K.tanh = subs(K.tanh, k, 1);
  Term.sinhcosh = K.sinhcosh*sinh(axb)^S*cosh(axb)^(n+2*k-1);
  Term.tanh = K.tanh*sinh(axb)/cosh(axb);
  %% integral calculation
  Fp = symsum(Term.sinhcosh, k, 1, N)+Term.tanh;
end
% =
function Fp = n_nonpos_half(n, a, b, x)
  ...
  clearassum;
  %% term calculations
  K.sinhcosh = prod(G(1:4))/((-a)^S(1)*(n+2*k));
  K.E = -sqrt(sym(2))*sympi*1i*prod(G(3:4))*S(2)/(2*a*G(5));
  K.F = -sqrt(sym(2))*1i*prod(G(3:5))*S(3)/(a*sympi);
  K.E = subs(K.E, k, 1);
  K.F = subs(K.F, k, 1);
  Term.sinhcosh = K.sinhcosh*sinh(axb)^S(1)*cosh(axb)^(n+2*k-1);
  Term.E = K.E*ellipticE(elliptic_args{:});
  Term.F = K.F*ellipticF(elliptic_args{:});
  %% integral calculation
  Fp = sym.zeros(2,1);
  Fp(1) = symsum(Term.sinhcosh, k, 1, N(6));
  Fp(2) = Term.E+Term.F;
  Fp = sum(Fp);
end
% =
function Fp = n_neg_odd(n, a, b, x)
  ...
  G = gamma(Arg).^[1; -S];
  %% term calculations
  K.sinhcosh = (-1)^(k+1)*prod(G(1:2))/(a^S*(n+2*k)*npermk(N, k));
  K.atan = (-1)^(-N)*sqrt(sympi)*G(2)/(a*factorial(N));
  K.atan = subs(K.atan, k, 1);
  Term.sinhcosh = K.sinhcosh*sinh(axb)^S*cosh(axb)^(n+2*k-1);
  Term.atan = K.atan*atan(sinh(axb));
  %% integral calculation
  Fp = symsum(Term.sinhcosh, k, 1, N)+Term.atan;
end
% =
function Fp = n_nonneg_even_one(n, a, b, x)
  ...
  N = n/2;
  %% term calculations
  K.sinh = nchoosek(n, k-1)/(2^((n-1)*S)*a^S*(n-2*k+2));
...
function Fp = n_nonneg_even_two(n, a, b, x)
  ...
  G = gamma(Arg).^[1; -S];
  %% term calculations
  K.sinhcosh = (-1)^k*prod(G(1:2))/(a^S*npermk(N, k)*(n-2*k+1));
  K.x = (-1)^N*sqrt(sympi)*G(2)/factorial(N);
  K.x = subs(K.x, k, 1);
  Term.sinh = K.sinhcosh*sinh(axb)^S*cosh(axb)^(n-2*k+1);
  Term.x = K.x*x;
  %% integral calculation
  Fp = symsum(Term.sinh, k, 1, N)+Term.x;
end
...
function Fp = n_nonneg_half(n, a, b, x)
  ...
  clearassum;
  %% term calculations
  K.sinhcosh = prod(G(1:4))/(a^S(1)*(n-2*k+1));
  K.E = sqrt(sym(2))*sympi*1i*prod(G(3:4))*S(2)/(2*a*G(5));
  K.F = -sqrt(sym(2))*1i*prod(G(3:5))*S(3)/(a*sympi);
  K.E = subs(K.E, k, 1);
  K.F = subs(K.F, k, 1);
...
function Fp = n_pos_odd_one(n, a, b, x)
  ...
  N = (n-1)/2;
  %% term calculations
  K = nchoosek(N, k-1)/(a^S*(2*k-1));
  Term = K*sinh(axb)^(2*k-1);
  %% integral calculation
...
function Fp = n_pos_odd_two(n, a, b, x)
  ...
  G = gamma(Arg).^[-1; S];
  %% term calculations
  K.sinhcosh = (-1)^k*prod(G(1:2))*npermk(N, k)/(a^S*(n-2*k+1));
  K.sinh = (-1)^(N+1)*G(2)*factorial(N)/(2*a*sqrt(sympi));
  K.sinh = subs(K.sinh, k, 1);
  Term.sinhcosh = K.sinhcosh*sinh(axb)^S*cosh(axb)^(n-2*k+1);
  Term.sinh = K.sinh*sinh(axb);
  %% integral calculation
  Fp = symsum(Term.sinhcosh, k, 1, N)+Term.sinh;
end
% =
