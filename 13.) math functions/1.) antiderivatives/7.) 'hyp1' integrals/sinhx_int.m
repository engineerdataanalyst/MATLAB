function [F f tF dF sF cF] = sinhx_int(n, a, b, x)
  % - formula for this integral:
  % ---------------------------
  %  /
  %  | (a*x+b)^n*sinh(a*x+b) dx
  %  /
  % ---------------------------
...
function Fp = n_neg_int(n, a, b, x)
  ...
  N.cosh = piecewise(isodd(n),  -n/2-1/2, ...
                     iseven(n), -n/2-1);
  P = -2*expression(N.sinh);
  Arg = [k-1; P];
  assume(Arg(2:3), 'real');
  S = [heaviside(Arg(1)); integer(Arg(2:3)/2)];
  clearassum;
  %% term calculations
  K.sinh = -1/(a^S(1)*npermk(-n-1, 2*k-1));
  K.cosh = -1/(a^S(1)*npermk(-n-1, 2*k));
  K.sinhint = (-1)^P(1)/(a*factorial(-n-1));
  K.coshint = (-1)^P(2)/(a*factorial(-n-1));
  Term.sinh = K.sinh*axb^(n+2*k-1)*sinh(axb);
  Term.cosh = K.cosh*axb^(n+2*k)*cosh(axb);
...
function Fp = n_nonpos_half(n, a, b, x)
  ...
  N.sinh = piecewise(isint(-n/2-1/4), -n/2-1/4, ...
                     isint(-n/2+1/4), -n/2+1/4);
  N.cosh = piecewise(isint(-n/2-3/4), -n/2-3/4, ...
                     isint(-n/2-1/4), -n/2-1/4);
  P = sym([1/4+1; 3/4]);
  Arg = [k-1; -expression(N.sinh); n+[1; 2*k+[0; 1]]];
  assume(Arg(2:3), 'real');
...
function Fp = n_nonneg_int(n, a, b, x)
  ...
  S = heaviside(k-1);
  %% term calculations
  K.sinh = -npermk(n, 2*k-1)/a^S;
  K.cosh = npermk(n, 2*k-2)/a^S;
  Term.sinh = K.sinh*axb^(n-2*k+1)*sinh(axb);
  Term.cosh = K.cosh*axb^(n-2*k+2)*cosh(axb);
  %% integral calculation
...
function Fp = n_nonneg_half(n, a, b, x)
  ...
  N.cosh = piecewise(isint(n/2+3/4), n/2+3/4, ...
                     isint(n/2+1/4), n/2+1/4);
  P = n+[-1/4; 5/4];
  Arg = [k-1; expression(N.sinh, [2 1]); -n+[2*k+[-1; -2]; 0]];
  assume(Arg(2:4), 'real');
  S = [heaviside(Arg(1)); integer(Arg(2:3))];
  G = gamma(Arg(4:6));
  clearassum;
