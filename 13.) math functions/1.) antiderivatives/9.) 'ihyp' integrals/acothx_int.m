function [F f tF dF sF cF] = acothx_int(n, a, b, x)
  % - formula for this integral:
  % ----------------------------
  %  /
  %  | (a*x+b)^n*acoth(a*x+b) dx
  %  /
  % ----------------------------
...
function [Fp fp dFp] = persistent_fun
  ...
  fp(n, a, b, x) = (a*x+b)^n*acoth(a*x+b);
  %% cases
  cases = [a == 0;
           n == -1 & a ~= 0;
           a ~= 0];
...
function Fp = n_neg_one(a, b, x)
  ...
  Term(3) = -log((a*b-a*(b-1))/(a*axb))^2/(4*a);
  Term(4) = log((a*b-a*(b+1))/(a*axb))^2/(4*a);
  Term(5) = -log(a*(axb-1)/(a*(b-1)-a*b))*log((a*b-a*(b-1))/(a*axb))/(2*a);
  Term(6) = log((axb-1)/axb)*log((a*b-a*(b-1))/(a*axb))/(2*a);
  Term(7) = log(a*(-axb-1)/(a*(-b-1)+a*b))*log((a*b-a*(b+1))/(a*axb))/(2*a);
  Term(8) = -log((a*b-a*(b+1))/(a*axb))*log((axb+1)/axb)/(2*a);
  Fp = sum(Term);
end
% =
function Fp = a_not_zero(n, a, b, x)
  ...
  Children(1) = scale_logs(Children(1), 1, 'Scale', -1);
  ...
  K = prod(Factors(Ind));
  Children(1) = sum(K*children(Children(1)/K));
  Children(1) = Simplify(Children(1), 2, IAC{:});
