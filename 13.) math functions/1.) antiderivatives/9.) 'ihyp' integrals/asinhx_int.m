function [F f tF dF sF cF] = asinhx_int(n, a, b, x)
  % - formula for this integral:
  % ----------------------------
  %  /
  %  | (a*x+b)^n*asinh(a*x+b) dx
  %  /
  % ----------------------------
...
function [Fp fp dFp] = persistent_fun
  ...
  fp(n, a, b, x) = (a*x+b)^n*asinh(a*x+b);
  %% cases
  cases = [a == 0;
           n == -1 & a ~= 0;
           a ~= 0];
...
function [Fp Diff] = n_neg_one(a, b, x)
  ...
  axb = a*x+b;
  Term = sym.zeros(3,1);
  Term(1) = asinh(axb)*log(1-exp(2*asinh(axb)))/a;
  Term(2) = -asinh(axb)^2/(2*a);
  Term(3) = polylog(2, exp(2*asinh(axb)))/(2*a);
  Fp = sum(Term);
  Diff = diff(Fp, x);
end
% =
function [Fp Diff] = a_not_zero(n, a, b, x)
  ...
  func = @(~) sinh_int(n+1, 1, 0, u, 'Method', 'two');
  ...
  Cosh = sqrt(1+Sinh^2);
  Coth = Cosh/Sinh;
  Fp = subs(Fu, [sinh(u) cosh(u) coth(u)], [Sinh Cosh Coth]);
  Fp = subs(Fp, u, U);
  ...
  %% differentiating the integeral (preliminaries)
  expr = diff(expr, x);
  expr([1 3 5 6]) = simplify(expr([1 3 5 6]), IAC{:});
  expr = subs(expr, axb2, axb^2);
  %% differentiating the integeral (case 2: [part 1])
  expr(2) = rewrite(expr(2), 'asin');
  expr(2) = Simplify(expr(2), 6:7, IAC{:});
  expr(2) = rewrite(expr(2), 'asinh');
  %% differentiating the integeral (case 2: [part 2])
  sublist = sqrt(axb*1i);
  subvals = sqrt(1i)*sqrt(axb);
  expr(2) = subs(expr(2), sublist, subvals);
  %% differentiating the integeral (case 2: [part 3])
  sublist = [1/8-sym(1i)/8; -1/4-sym(1i)/4];
  subvals = sqrt(sym(2))*[sqrt(sym(-1i))/8; -sqrt(sym(1i))/4];
  expr(2) = subs(expr(2), sublist, subvals);
  %% differentiating the integeral (case 4: [part 1])
  expr(4) = rewrite(expr(4), 'asin');
  expr(4) = Simplify(expr(4), 6:7, IAC{:});
  expr(4) = rewrite(expr(4), 'asinh');
  %% differentiating the integeral (case 4: [part 2])
  sublist = sqrt(axb*1i);
  subvals = sqrt(1i)*sqrt(axb);
  expr(4) = subs(expr(4), sublist, subvals);
  %% differentiating the integeral (case 4: [part 3])
  sublist = [1/8-sym(1i)/8; 1/4+sym(1i)/4];
  subvals = sqrt(sym(2))*[sqrt(sym(-1i))/8; sqrt(sym(1i))/4];
  expr(4) = subs(expr(4), sublist, subvals);
  %% differentiating the integeral (converting to piecewise)
