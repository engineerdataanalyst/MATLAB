function [F f tF dF sF cF] = acoshx_int(n, a, b, x)
  % - formula for this integral:
  % ----------------------------
  %  /
  %  | (a*x+b)^n*acosh(a*x+b) dx
  %  /
  % ----------------------------
...
function [Fp fp dFp] = persistent_fun
  ...
  fp(n, a, b, x) = (a*x+b)^n*acosh(a*x+b);
  %% cases
  cases = [a == 0;
           n == -1 & a ~= 0;
           a ~= 0];
...
function Fp = n_neg_one(a, b, x)
  ...
  axb = a*x+b;
  Term = sym.zeros(3,1);
  Term(1) = acosh(axb)*log(1+exp(-2*acosh(axb)))/a;
  Term(2) = acosh(axb)^2/(2*a);
  Term(3) = -polylog(2, -exp(-2*acosh(axb)))/(2*a);
  Fp = sum(Term);
end
% =
function Fp = a_not_zero(n, a, b, x)
  ...
  axb = a*x+b;
  axb2 = expand(axb^2); %#ok<NASGU> 
  sqrt_axb = prod(sqrt(axb+[-1 1]).');
  S = heaviside(k-1);
  %% integration by parts
  sublist = sym(children(sqrt_axb));
  subvals = [sqrt(axb^2-1) 1];
  U = acosh(axb);
  ...
  func = @(~) cosh_int(n+1, 1, 0, u, 'Method', 'two');
  ...
  Fp = subs(Fu, [sinh(u) cosh(u)], [Sinh Cosh]);
  Fp = subs(Fp, u, U);
