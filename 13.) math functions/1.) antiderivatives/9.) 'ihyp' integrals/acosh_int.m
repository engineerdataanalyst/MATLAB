function [F f tF dF sF cF] = acosh_int(n, a, b, x)
...
function Fp = a_not_zero(n, a, b, x)
  ...
  func = @(~) sinhx_int(n, 1, 0, u);
  Iu = mapSymType(Iu, 'int', func);
  %% %% back substituting for x
  Sinh = prod(sqrt(axb+[-1; 1]));
  Cosh = axb;
  Fp = subs(Iu, [sinh(u) cosh(u)], [Sinh Cosh]);
  Fp = subs(Fp, u, U);
  %% modifying the integral
  % ... new code for this section coming soon!!!!
end
% =
