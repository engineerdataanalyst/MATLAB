function [F f tF dF sF cF] = acschx_int(n, a, b, x)
  % - formula for this integral:
  % ----------------------------
  %  /
  %  | (a*x+b)^n*acsch(a*x+b) dx
  %  /
  % ----------------------------
...
function [Fp fp dFp] = persistent_fun
  ...
  fp(n, a, b, x) = (a*x+b)^n*acsch(a*x+b);
  %% cases
  cases = [a == 0;
           n == -1 & a ~= 0;
           a ~= 0];
...
function [Fp Diff] = n_neg_one(a, b, x)
  ...
  Term = sym.zeros(3,1);
  Term(1) = -acsch(axb)*log(1-exp(-2*acsch(axb)))/a;
  Term(2) = -acsch(axb)^2/(2*a);
  Term(3) = polylog(2, exp(-2*acsch(axb)))/(2*a);
  Fp = sum(Term);
...
function [Fp Diff] = a_not_zero(n, a, b, x)
  ...
  Iu = simplify(Iu, IAC{:});
  %% integral calculation
  func = @(~) csch_int(n+1, 1, 0, u, 'Method', 'two');
  ...
  Sinh = 1/Csch;
  Cosh = sqrt(1+Sinh^2);
  Cosh = simplify(simplifyFraction(Cosh), IAC{:});
  Cosh = subs(Cosh, axb2, axb^2);
  Coth = Cosh/Sinh;
  Coth = simplify(simplifyFraction(Coth), IAC{:});
  Coth = subs(Coth, axb2, axb^2);
  Fp = subs(Fu, [sinh(u) cosh(u) csch(u) coth(u)], [Sinh Cosh Csch Coth]);
  Fp = subs(Fp, u, U);
