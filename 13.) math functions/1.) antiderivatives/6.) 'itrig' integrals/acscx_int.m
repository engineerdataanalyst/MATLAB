function [F f tF dF sF cF] = acscx_int(n, a, b, x)
  % - formula for this integral:
  % ---------------------------
  %  /
  %  | (a*x+b)^n*acsc(a*x+b) dx
  %  /
  % ---------------------------
...
function [Fp Diff] = n_neg_one(a, b, x)
  ...
  axb = a*x+b;
  Term = sym.zeros(3,1);
  Term(1) = -acsc(axb)*log(1-exp(2i*acsc(axb)))/a;
  Term(2) = 1i*acsc(axb)^2/(2*a);
  Term(3) = 1i*polylog(2, exp(2i*acsc(axb)))/(2*a);
  Fp = sum(Term);
...
function [Fp Diff] = a_not_zero(n, a, b, x)
  ...
  func = @(~) csc_int(n+1, 1, 0, u, 'Method', 'two');
  ...
  Sin = 1/Csc;
  Cos = sqrt(1-Sin^2);
  Cos = simplify(simplifyFraction(Cos), IAC{:});
  Cos = subs(Cos, axb2, axb^2);
  Cot = Cos/Sin;
  Cot = simplify(simplifyFraction(Cot), IAC{:});
  Cot = subs(Cot, axb2, axb^2);
  Fp = subs(Fu, [sin(u) cos(u) csc(u) cot(u)], [Sin Cos Csc Cot]);
  Fp = subs(Fp, u, U);
