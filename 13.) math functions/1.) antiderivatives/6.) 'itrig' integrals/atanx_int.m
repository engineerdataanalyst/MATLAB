function [F f tF dF sF cF] = atanx_int(n, a, b, x)
  % - formula for this integral:
  % ---------------------------
  %  /
  %  | (a*x+b)^n*atan(a*x+b) dx
  %  /
  % ---------------------------
...
function Fp = n_neg_one(a, b, x)
  ...
  axb = a*x+b;
  Term = sym.zeros(2,1);
  Term(1) = 1i*polylog(2, -axb*1i)/(2*a);
  Term(2) = -1i*polylog(2, axb*1i)/(2*a);
  Fp = sum(Term);
end
% =
function Fp = a_not_zero(n, a, b, x)
  ...
  Children(1) = mapSymType(Children(1), 'acot | acoth', func);
  Children(1) = split_logs(Children(1));
  %% modifying the integral (case 1: [part 6])
  sublist = children(Children(1), 3);
  ...
  sublist = [(-1).^(n/2+[1; 1/2]).*[log(axb); atan(axb)]; powers(4)];
  subvals = [-(-1).^(n/2+[0; -1/2]).*[log(axb); atan(axb)]; powers(4)];
  ...
  subvals(2:3) = -1./combine(-1./subvals(2:3), IAC{:});
  Children(1) = subs(Children(1), sublist, subvals);
  %% modifying the integral (case 2: [part 6])
  Children(1) = split_logs(Children(1));
  %% modifying the integral (case 2: [part 7])
  assume(n/2+1/2 > 0);
  extra = (-1)^(k+1)*b*heaviside(k-n/2-1/2)*integer(n/2+1/2);
  clearassum;
  sublist = (-1)^k*axb^(n-2*k+2);
  subvals = sublist+extra;
  Children(1) = subs(Children(1), sublist, subvals);
  %% modifying the integral (case 2: [part 8])
  expr{2} = sum(Children);
