function [F f tF dF sF cF] = acos_int(n, a, b, x)
...
function Fp = a_not_zero(n, a, b, x)
  ...
  for p = 1:4
    ...
    subvals = sublist;
    Old_Term = arrayfun(func, sublist);
    switch p
      case {1 4}
        New_Term = Old_Term*(-1)^(k+1)/((-1)^k*a^S);
      case {2 3}
        New_Term = Old_Term.*(-1).^(k+[0; 1])./((-1).^(k+[1; 0])*a^S);
    end
    subvals = -a*subs(subvals, Old_Term, New_Term);
    expr(p) = sum(subs(Children, sublist, subvals));
  end
  %% modifying the integral (negative one)
  sublist = findSymType(expr, 'power').';
  sublist = sublist(13:18);
  [~, subvals] = power_parts(sublist);
  subvals = -(-1).^(subvals+[-ones(2,1); 1; -ones(3,1)]);
  expr = subs(expr, sublist, subvals);
