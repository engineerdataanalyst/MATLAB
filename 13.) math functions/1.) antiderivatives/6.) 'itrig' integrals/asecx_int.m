function [F f tF dF sF cF] = asecx_int(n, a, b, x)
  % - formula for this integral:
  % ---------------------------
  %  /
  %  | (a*x+b)^n*asec(a*x+b) dx
  %  /
  % ---------------------------
...
function [Fp Diff] = n_neg_one(a, b, x)
  ...
  axb = a*x+b;
  Term = sym.zeros(3,1);
  Term(1) = -asec(axb)*log(1+exp(2i*asec(axb)))/a;
  Term(2) = 1i*asec(axb)^2/(2*a);
  Term(3) = 1i*polylog(2, -exp(2i*asec(axb)))/(2*a);
  Fp = sum(Term);
...
function [Fp Diff] = a_not_zero(n, a, b, x)
  ...
  func = @(~) sec_int(n+1, 1, 0, u, 'Method', 'two');
  ...
  Cos = 1/Sec;
  Sin = sqrt(1-Cos^2);
  Sin = simplify(simplifyFraction(Sin), IAC{:});
  Sin = subs(Sin, axb2, axb^2);
  Tan = Sin/Cos;
  Tan = simplify(simplifyFraction(Tan), IAC{:});
  Tan = subs(Tan, axb2, axb^2);
  Fp = subs(Fu, [sin(u) cos(u) sec(u) tan(u)], [Sin Cos Sec Tan]);
  Fp = subs(Fp, u, U);
