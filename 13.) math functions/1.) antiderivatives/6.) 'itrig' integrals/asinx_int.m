function [F f tF dF sF cF] = asinx_int(n, a, b, x)
  % - formula for this integral:
  % ---------------------------
  %  /
  %  | (a*x+b)^n*asin(a*x+b) dx
  %  /
  % ---------------------------
...
function Fp = n_neg_one(a, b, x)
  ...
  axb = a*x+b;
  Term = sym.zeros(3,1);
  Term(1) = asin(axb)*log(1-exp(2i*asin(axb)))/a;
  Term(2) = -1i*asin(axb)^2/(2*a);
  Term(3) = -1i*polylog(2, exp(2i*asin(axb)))/(2*a);
  Fp = sum(Term);
end
% =
function Fp = a_not_zero(n, a, b, x)
  ...
  func = @(~) sin_int(n+1, 1, 0, u, 'Method', 'two');
  ...
  Cos = sqrt(1-Sin^2);
  Cot = Cos/Sin;
  Fp = subs(Fu, [sin(u) cos(u) cot(u)], [Sin Cos Cot]);
  Fp = subs(Fp, u, U);
  ...
  sublist = [(-1)^(n/2); gamma(-n/2)];
  subvals = [-(-1)^(n/2+1); factorial(-n/2-1)];
  Children(1) = subs(Children(1), sublist, subvals);
