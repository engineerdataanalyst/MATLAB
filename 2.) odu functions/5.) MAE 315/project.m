...
for j = 2:n
% calculating lhs
  lhs(1,1) = (lambda+2)/lambda*r(1)^2+rhalf(1)^2;
  lhs(1,2) = -(rhalf(1)^2+r(1)^2);
  for k = 2:10
    lhs(k,k-1) = -rhalf(k-1)^2;
    lhs(k,k) = rhalf(k-1)^2+2*r(k)^2/lambda+rhalf(k)^2;
    lhs(k,k+1) = -rhalf(k)^2;
  end
  lhs(11,10) = -(rhalf(10)^2+r(11)^2);
  lhs(11,11) = rhalf(10)^2+(2*h*lambda*dr+k*lambda+2*k)/(k*lambda)*r(11);
% calculating rhs
  rhs(1) = -((lambda-2)/lambda*r(1)^2+rhalf(1)^2)*T(1,j-1)+(r(1)^2+rhalf(1)^2)*T(2,j-1);
  for k = 2:10
    rhs(k) = rhalf(k-1)^2*T(k-1,j-1)-(rhalf(k-1)^2-2*r(k)^2/lambda+rhalf(k)^2)*T(k,j-1)+rhalf(k)^2*T(k+1,j-1);    
  end
  rhs(11) = (rhalf(10)^2+r(11)^2)*T(10,j-1)-(rhalf(10)^2+(2*h*lambda*dr+k*lambda-2*k)/(k*lambda)*r(11)^2)*T(11,j-1)+4*h*r(11)^2*dr/k*Tinf;
% calculating T
  T(1:11,j) = lhs\rhs;  
end
