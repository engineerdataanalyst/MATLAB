function str = fix_int(str)
  % -----------------------------------
  % - converts a string to a valid
  %   MuPAD syntax for the int function
  % -----------------------------------
...
function [old new finish] = compute_args(str, start)
  ...
  %% compute the old and new int argument strings
  if length(Args) == 4 && valid_int
    old = str(start:finish);
    new = sprintf('int(%s,%s=%s..%s)', Args{:});
  else
    old = '';
    new = '';
  end
