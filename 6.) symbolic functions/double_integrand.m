function a = double_integrand(a, ind, options)
  % --------------------------------
  % - doubles the integrand
  ...
  even = @(body, var, lower_limit, upper_limit) ...
         isevenfun(body) && ...
         isAlways(lower_limit == -upper_limit, 'Unknown', 'false');
  ...
  for k = 1:length(sublist)
    ...
    if even(body, var, lower_limit, upper_limit)
      subvals(k) = 2*int(body, var, 0, upper_limit, Hold{:});
      if Combine(k)
