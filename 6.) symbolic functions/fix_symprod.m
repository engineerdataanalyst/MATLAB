function str = fix_symprod(str)
  % ---------------------------------------
  % - converts a string to a valid
  %   MuPAD syntax for the symprod function
  % ---------------------------------------
...
function [old new finish] = compute_args(str, start)
  ...
  %   for the symprod function
  % --------------------------
  
  %% compute the symprod arguments
  Args = {};
  finish = start+7;
  str_len = length(str);
  ...
  %% compute the old and new symprod argument strings
  if any(length(Args) == [2 4]) && valid_symprod
    old = str(start:finish);
    if length(Args) == 2
      new = sprintf('product(%s,%s)', Args{:});
    else
      new = sprintf('product(%s,%s=%s..%s)', Args{:});
    end
  else
    old = '';
    new = '';
