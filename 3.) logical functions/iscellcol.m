function bool = iscellcol(a, classname, options)
  % --------------------------
  % - returns true if an array
  %   is a cell column vector
  %   and/or if an array
  ...
  if Mode == "cell"
    ...
    func{1} = @(arg) isColumn(a, 'cell', options{1}{:});
    ...
    if Mode == "values"
      ...
      func{1} = @(arg) isColumn(arg, 'cell', options{1}{:});
    end
    % second function handle
    if nargin == 1
      func{2} = @(arg) isColumn(arg, options{2}{:});
    else
      func{2} = @(arg) isColumn(arg, classname, options{2}{:});
    end
  end
  %% check the array
