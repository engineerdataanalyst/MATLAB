function bool = iscellmatrix(a, classname, options)
  % ---------------------------
  ...
  if Mode == "cell"
    ...
    func{1} = @(arg) isMatrix(a, 'cell', options{1}{:});
    ...
    if Mode == "values"
      ...
      func{1} = @(arg) isMatrix(arg, 'cell', options{1}{:});
    end
    % second function handle
    if nargin == 1
      func{2} = @(arg) isMatrix(arg, options{2}{:});
    else
      func{2} = @(arg) isMatrix(arg, classname, options{2}{:});
    end
  end
  %% check the array
