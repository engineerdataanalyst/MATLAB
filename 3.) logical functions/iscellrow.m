function bool = iscellrow(a, classname, options)
  % --------------------------
  % - returns true if an array
  %   is a cell row vector
  %   and/or if an array
  ...
  if Mode == "cell"
    ...
    func{1} = @(arg) isRow(a, 'cell', options{1}{:});
    ...
    if Mode == "values"
      ...
      func{1} = @(arg) isRow(arg, 'cell', options{1}{:});
    end
    % second function handle
    if nargin == 1
      func{2} = @(arg) isRow(arg, options{2}{:});
    else
      func{2} = @(arg) isRow(arg, classname, options{2}{:});
    end
  end
  %% check the array
