function bool = isTextCol(a, classname, options)
  % ----------------------------
  % - returns true if an array
  %   is a column vector of text
  % ----------------------------
  ...
  fields = string.empty;
  if isfield(options{1}, 'ArrayLen')
    fields = [fields "ArrayLen"];
  end
  ...
  % remove the necessary fields
  fields = string.empty;
  if isfield(options{3}, 'TextLen')
    fields = [fields "TextLen"];
  end
  ...
  if isfield(options{3}, 'ArrayLen')
    old_fields = [old_fields "ArrayLen"];
    new_fields = [new_fields "CellLen"];
  end
  ...
  if iscell(a)
    ...
    string_case = @(arg) isStringCol(arg, options{2}{:});
  end
  ...
  for k = 1:length(classname)
    ...
    switch classname(k)
      ...
      case "string"
        bool = string_case(a);
      otherwise
        if iscellcol(a, options{3}{:})
          cell_type = extractAfter(classname(k), "cell of ");
          switch cell_type
            case "char"
              bool = all(cellfun(char_case, a));
            case "string"
              bool = all(cellfun(string_case, a));
            otherwise
              bool = all(cellfun(char_or_string_case, a));
          end
        end
    end
